...
function getNameLastNameManager(String $ordre=''):array{
    try{
            ...
            $connexion->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            if($ordre=="decroissant"){
                 $requete=$connexion->prepare("SELECT prenom,nom FROM members WHERE access='MANAGER' ORDER BY nom DESC");
            }
            else{
                $requete=$connexion->prepare("SELECT prenom,nom FROM members WHERE access='MANAGER' ORDER BY nom ASC");
