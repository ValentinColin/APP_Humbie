...
function resultsByManager($id,$nbr =50){
    ...
    foreach($datas as $element)
    {
        ...
        $datas[$i]['results'] = $array_results[0];
        $i +=1;
    }
...
function newResults($id, $result, $typeTest){
        $bdd = login_bdd();
        $req = $bdd->query('SELECT  `id_session`, `date`
                        FROM `test_board` JOIN `members` ON test_board.id_user = members.id
                        WHERE `id` = ' . $id . ' ORDER BY  `date` DESC LIMIT 1');

        $datas = $req->fetchAll();
        if ($datas != null){
            $datas = $datas[0];
            $id_session = $datas['id_session'];
            $date = $datas['date'];
        ...
        /**
        echo 'INSERT INTO `test_board`(`type`, `id_session`, `id_user`, `result`, `city`) VALUES (' . $typeTest . ',' . $id_session . ',' . $id . ',' . $result . ', "Paris")';
        $req = $bdd->query('INSERT INTO `test_board`(`type`, `id_session`, `id_user`, `result`, `city`) VALUES (' . $typeTest . ',' . $id_session . ',' . $id . ',' . $result . ', Paris)');
        $req->closeCursor();
        */

        $sql = 'INSERT INTO `test_board`(`id_type`, `id_session`, `id_user`, `result`, `city`) VALUES (' . $typeTest . ',' . $id_session . ',' . $id . ', :result, "Paris")';
        $req = $bdd->prepare($sql);
        print_r($result);
